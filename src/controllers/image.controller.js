...
import multer from 'multer';
import AppError from '../utils/appError.js';
import { deleteFromCloudinary, listCloudinaryImages, uploadToCloudinary } from '../utils/cloudinary.js';
...
export const uploadImage = async (req, res, next) => {
  try {
    ...
    if (!req.body.file) return next(new AppError('No file uploaded', 400));
    const url = await uploadToCloudinary(req.body.file);
    res.status(201).json({ status: 'success', url });
  } catch (err) {
    next(err);
  }
